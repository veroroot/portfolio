...
FastAPI 기반 서비스 구축, 데이터 구조 설계, AI 모델 서빙 등 서버 개발 중심으로 경험을 쌓아왔습니다.

다양한 AI 프로젝트(자연어, 시계열, 비전, 영상)를 통해 모델 연계 및 데이터 연계 방안에 대한 노하우를 학습하였습니다.

기획, 데이터 분석, 모델 구축, 백엔드 경험을 토대로 연구원, 개발자 뿐만 아니라 비개발자 간 커뮤니케이션 능력을 개선시켜 왔습니다.

사업 제안 단계부터 개발, 감리 및 인증 테스트 대응 경험을 통해 프로젝트 전체 프로세스 경험을 보유하고 있습니다.

- 📍 Seoul, KR
- ✉️ <veroroot@gmail.com>
...
- Restful API, WebSocket, MQTT, Pub/Sub
- SQLAlchemy, SQLModel
- JWT, OAuth2, RBAC 인증/권한
- Multi Processing / Threads

### AI(DL/ML, Preprocessing, NPU etc.)
...
- Gitlab, Jenkins, Harbor (CI/CD)

#### Cloud

- AWS EC2 / ECS on Fargate / Lambda
- AWS API Gateway, ALB
...
- AWS RDS
- AWS ElastiCache, AWS DocumentDB
- AWS S3
...
계약 수명 주기 관리 시스템 기반 문서 업로드 RAG를 통해 법률 기반 계약서 비교, 위험조항 검토, 계약서 생성 등을 MS Word Add In / Web 기반 서비스를 구축햇습니다.
...
영상 캡처 보드에서 실시간 영상 데이터를 받아 웹 클라이언트로 표출하는 실시간 모니터링 시스템을 구축했습니다.
...
- 캡처보드 인식 및 실시간 영상 제공 프로세스
- 딥러닝 모델 기반 실시간 영상 제공 프로세스
...
초대 및 권한 기능을 포함한 다중 워크스페이스/프로젝트 구조 이미지 저장 관리시스템을 구축했습니다.
...
수요기업 대상 스마트 공장 구축을 위한 사전 기능 및 AI 기능 개발하였습니다.
...
![스마트공장 아키텍처](assets/projects/smartfactory/)
...
- 화질 개선 딥러닝 모델 기반의 Denoise 기능을 NPU 및 웹 기반 서버를 통해 수요기업에 솔루션 형태로 제공하였습니다.
...
- 지하공동구 스마트 관리를 위해 수집되는 센싱 데이터 등 다양한 기기에서 발생하는 빅데이터를 수집 및 관리하는 체계 시스템을 개발하였습니다.
